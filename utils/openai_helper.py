...
def generate_summary(chunk_text, user_question):
    prompt = f"Assume you are an Github Expert. Summarise the following documentation to extract useful information to answer user questions {user_question}. Give priority to information that can help in answering: {user_question}.  Documentation: {encoding.decode(chunk_text)}"
    messages = [{"role": "system", "content": "This is Github documentation summarization."}]    
...
def consolidate_summaries(summary_list, user_question):
    prompt = f"Assume you are an Github expert. Consolidate these documentation summaries mentioned under `Documentation` into a single big summary. Give priority to information that can help in answering: {user_question}."
...
Assume you are an assistant with knowledge about Github. Answer any questions about Dcumentation with the following format -
